...
module Microsoft.Research.CRNEngine.Tests.SimulatorsRegressionTests
...
let compare (expected : Result<float>) (actual : Result<float>) (eventTimes : float list) isInterpolated =
  ...
  (* CS: when a CRN events is simulated, the simulators may print multiple datapoints at the same time stamp.
         For example, simulating "init X 1 @ 10" produces two datapoints at time 10 with Oslo, 
  ...
  let formatTimestampsRev (rows : Row<float> list) = 
    match rows with
    | [] -> []
    | r::rs -> let rec filterRowLoop (next : Row<float>) (rows : Row<float> list) acc =
                match rows with 
                | []    -> next::acc
                | r::rs -> if next.time <> r.time && List.forall (notInRange r.time) evRanges
                            then filterRowLoop r rs (next::acc) // order is reversed here
                              ...
                              if isInterpolated 
                                then filterRowLoop next rs acc
                                else filterRowLoop r rs acc
               in filterRowLoop r rs []
  
  let colName (c : Column<float>) = c.name
  let mkTable (r:Result<float>) = r.table
  ...
  let renameTableCols (f:string->string) (t:Table<float>):Table<float> =
    let renameCol (c:Column<float>) = { c with name = f c.name }
  ...
  (** /CME hack**)
  // strip any [] around species
  let plotHack (colName:string) = colName.Replace("[", "").Replace("]", "")
  
  let aTab = mkTable actual   |> renameTableCols plotHack
  ...
  let eColumns  = aTab.columns |> List.map (fun (c:Column<float>) -> c.name)
  let iTab      = if isInterpolated
                    then aTab
                    else
                      let iRowsRev  = Row.interpolate_reverse Row<float>.interpolate_float aRows eTimes
                      let iTabDups  = Table.from_rows_reverse eColumns iRowsRev
                      let iResults  = Result<_>.create Instance.empty iTabDups
                      mkTable iResults
  ...
  let scale = aTab.columns 
              |> List.map (fun (c:Column<float>) -> c.values |> findMax) 
              |> findMax
  ...
  let findColumn (name:string) (tab: Table<float>) = List.tryFind (fun (c:Column<float>) -> c.name = name) tab.columns

  let testICol (iCol : Column<float>) =
  ...
  let testLoop errorMsg (c : Column<float>) =
    match errorMsg with 
    | None -> testICol c
    | _    -> errorMsg
  ...
  if aTab.columns.Length <> eTab.columns.Length
    then Some "Column lengths differ.\n"
      ...
      let getColumnName ( c : Column<float> ) = c.name
...
let run_single expected (actualCrn:Crn) isInterpolated =
    //debug ("Running simulator regression test: " + test_name)
    let actuals = actualCrn.simulate_case()
    let actual   = if actuals.Length <> 1
                    then failwith "Simulation testing for sweeps is not supported yet."
                    else actuals.Head
    let env        = Parameters.to_env actualCrn.settings.parameters
    let eventTimes = 
      actualCrn.initials
      |> List.choose (fun (i:Initial<Species,Value>) -> 
        match i.time with
        | Some t -> 
          match Expression.eval (Environment.find env) t with
          | 0.0 -> None
          | i   -> Some i
        | None -> None
      )
    ...
    let stripParenthesesFromHeaders (actual:Result<float>) = 
      { actual with table = 
                      {actual.table with columns = List.map (fun (c:Column<float>) -> {c with name = String.filter (fun (ch : char) -> not (List.contains ch ['('; ')'])) c.name}) actual.table.columns }}
    let actual'   = stripParenthesesFromHeaders actual
    let expected' = stripParenthesesFromHeaders expected

    compare expected' actual' eventTimes isInterpolated

type SimulatorFlag = Oslo | OsloStiff | Sundials | SundialsStiff | Lna | LnaStiff
    
let run testName (sim, crn : Crn) simKind : string option =
  let simData = Parser.from_string Result<float>.tsv_parser sim
  
  let simKind, isStiff, isInterpolated = 
    match simKind with 
        | Oslo          -> Simulator.Oslo,     false,  false
        | OsloStiff     -> Simulator.Oslo,     true,   false
        | Sundials      -> Simulator.Sundials, false,  true
        | SundialsStiff -> Simulator.Sundials, true,   true
        | Lna           -> Simulator.LNA,      false,  false
        | LnaStiff      -> Simulator.LNA,      true,   false
  
  let iSimulation   = if isInterpolated
                        then {crn.settings.simulation with times=simData.table.times}
                        else crn.settings.simulation
  let iDeterministic = {crn.settings.deterministic with stiff        = isStiff;
                                                        reltolerance = 1e-5}
  let simCrn  = {crn with settings = 
                            { crn.settings with simulator     = simKind
                                                simulation    = iSimulation
                                                deterministic = iDeterministic}}
                                                  
  match run_single simData simCrn isInterpolated with
  | None       -> None
  | Some error -> let stiff = if isStiff then " stiff" else ""
                  let kind  = simKind.to_string
...
let testSingleCrn crnDir selectedSimulators errorMsgs (crnFile:string) : string list= 
  let simDirName = Path.GetFileNameWithoutExtension crnFile + "_simulation"
  let simDir     = crnDir +/ simDirName
  if not (Directory.Exists simDir)
    then  
      ...
      let runSimulationTest simErrors simFile = 
        ...
        let runIfSelected k = 
          if List.contains k selectedSimulators
            then run testName test k
            else None
        let maybeError = 
          match simKind with
          | "deterministic"       -> runIfSelected Oslo
          | "sundials"            -> runIfSelected Sundials
          | "lna"                 -> runIfSelected Lna
          | "deterministicstiff"  -> runIfSelected OsloStiff
          | "sundialsstiff"       -> runIfSelected SundialsStiff
          | "lnastiff"            -> runIfSelected LnaStiff
...
let runSlowTestsOn simFlags =
  // load CRN file names  
  let projectDir = Directory.GetParent(Directory.GetCurrentDirectory()).Parent.Parent.FullName.Trim [|' ';'\r';'\n';'\t'|]
  ...
  match List.fold (testSingleCrn testsDir simFlags) [] crns with
  | []    -> ()
...
(** run Oslo, Sundials and LNA (stiff and non-stiff) on CRNs generated from Visual DSD programs.
    ...
    The total running time is ~90 minutes (CS: on my machine, as of 10/03/2017). **)


(* I've disabled some of these tests as they're taking a very long time e.g. Sundials - all was 651 minutes last run *)

(*[<Trait("Category", "Slow")>]
[<Fact(DisplayName="(slow) Simulation regressions - Oslo - all")>]
let Oslo_all () = runSlowTestsOn [Oslo]*)

[<Trait("Category", "Slow")>]
[<Fact(DisplayName="(slow) Simulation regressions - Oslo stiff - all")>]
let oslo_stiff_all () = runSlowTestsOn [OsloStiff]


(*[<Trait("Category", "Slow")>]
[<Fact(DisplayName="(slow) Simulation regressions - Sundials - all")>]
let sundials_all () = runSlowTestsOn [Sundials]*)

(*[<Trait("Category", "Slow")>]
[<Fact(DisplayName="(slow) Simulation regressions - LNA - all")>]
let lna_all () = runSlowTestsOn [Lna]*)

[<Trait("Category", "Slow")>]
[<Fact(Skip="LNA behaviour has been changed, deciding how to resolve with Neil", DisplayName="(slow) Simulation regressions - LNA stiff - all")>]
let lna_stiff_all () = runSlowTestsOn [LnaStiff]

[<Trait("Category", "Slow")>]
[<Fact(DisplayName="(slow) Simulation regressions - Sundials stiff - all")>]
let sundials_stiff_all () = runSlowTestsOn [SundialsStiff]


(** run the regression tests on a restricted subset of CRNs, so that it can be run within one minute **)
let runQuickTestsOn sims = 
  let projectDir   = Directory.GetParent(Directory.GetCurrentDirectory()).Parent.Parent.FullName.Trim [|' ';'\r';'\n';'\t'|]
  ...
  match List.fold (testSingleCrn testsPath sims) [] crns with
  | []    -> ()
  | errs  -> failwith (String.concat "\n" errs)

[<Trait("Category", "Slow")>]
[<Fact(DisplayName="(slow) Simulation regressions - Oslo - subset")>]
let oslo_subset () = runQuickTestsOn [Oslo]

[<Trait("Category", "Slow")>]
[<Fact(DisplayName="(slow) Simulation regressions - Sundials - subset")>]
let sundials_subset () = runQuickTestsOn [Sundials]

[<Trait("Category", "Slow")>]
[<Fact(Skip="LNA behaviour has been changed, deciding how to resolve with Neil", DisplayName="(slow) Simulation regressions - LNA - subset")>]
let lna_subset () = runQuickTestsOn [Lna]

[<Trait("Category", "Slow")>]
[<Fact(DisplayName="(slow) Simulation regressions - Oslo stiff - subset")>]
let oslo_stiff_subset () = runQuickTestsOn [OsloStiff]

[<Trait("Category", "Slow")>]
[<Fact(DisplayName="(slow) Simulation regressions - LNA stiff - subset")>]
let ``LNA stiff - subset`` () = runQuickTestsOn [Oslo]
...
let rec ep kp e : string = // expression printer
  ...
  | Expression.t.Round x                       -> "round(" + ep kp x + ")"
  | Expression.t.Ceiling x                     -> "ceiling(" + ep kp x + ")"
  | Expression.t.Floor x                       -> "floor(" + ep kp x + ")"
  | Expression.t.If (_, e1, e2)                -> failwith "Unexpected if-then-else statement."
(** /copied and pasted **)
...
let to_string_bindings (a:Assignment) = 
  let varsTuple = tuple_to_string a.variables
...
[<Fact(DisplayName="(slow) Simulation regressions - CME - all")>]
[<Trait("Category", "Slow")>]
let cme_all () =
  // get test file names
  let projectDir   = Directory.GetParent(Directory.GetCurrentDirectory()).Parent.Parent.FullName.Trim [|' ';'\r';'\n';'\t'|]
  ...
  // test a cnrFile
  let testSingleCrn (crnFile:string) : string list option = 
    let simDirName = Path.GetFileNameWithoutExtension crnFile + "_simulation"
    ...
    if not (Directory.Exists simDir)
      then  
        ...
        // plot printer 
        let pp          = ep (fun k -> match k with 
                                       | Key.Time          -> "[time]"
                                       | Key.Species sp    -> Species.to_string sp
                                       | Key.Parameter par -> par
                                       | Key.Rate r        -> failwith <| sprintf "Unexpected rate expression %s" r)
        ...
        let mkTestFileName stiffPrefix kind (actualInstance:Instance<Functional>) = 
        ...
        let loadCmeTable testFile (actualInstance:Instance<Functional>) = 
          let contents  = File.ReadAllText(testFile)
          let table     = Parser.from_string Result<Point>.tsv_parser contents
        ...
        let simulateSweep isStiff (i:Instance<Functional>) =
          // run CME on a sweep
          let plot = plotNames.Head
          let testCrn = if isStiff
                          then stiffCrn
                          else crn
          let cme, result = (testCrn.substitute i.environment).simulate_cme_state ()
          let aMarginalsRaw = Probabilities.probability_map None cme.probabilities plot
          let aData = Result<_>.create i (Table<float>.point_to_float result)

          // store the marginal distributions in a Result
          ...
          let aMarginals      = Result<_>.create i aMarginalsTable
        ...
        let runTest isStiff = 
          ...
          |> List.choose
              (fun (testFile, expected, actual) ->  
                ...
                let notInterpolated = false
                match compare eDataInRange actual [] notInterpolated with
                | None -> None
