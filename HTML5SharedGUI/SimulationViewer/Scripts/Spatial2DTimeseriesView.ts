...
import * as template from 'raw-loader!../html/spatial-simulation-2d.html';
ko.components.register("spatial-simulation-2d", {
    template: template,
    viewModel: {
        createViewModel: (params, componentInfo) => {
            if (componentInfo.element == null)
                throw "Attempt to create a Spatial2D VM for null";
...
class InstanceData {
    ...
    public RawData: KnockoutObservable<number[][][]> = ko.observable([]);
}

// The VM for this control; it's fairly straightforward. Note that it supports SVG export. Also, it's important to note that in "data" here, there is an array with as many elements as the values of x-axis, and each of those has as many elements as the values of y-axis. 
class ViewModel {
...
// This class implements the control. Note that it supports SVG exports.
export class Viewer implements Rx.IObserver<Framework.IVisualizationUpdateMessage> {
    ...
    // Construct the control inside the specified element.
    Bind(div: HTMLElement) {
        if (div == null)
            throw "Attempt to bind Spatial2DTimeseriesView to null";
        ko.cleanNode(div);
    ...
    public onNext(update: Framework.IVisualizationUpdateMessage) {
        ...
        switch (update.MessageType) {
            ...
            case Framework.VisualizationUpdateMessageType.SimulationStep2D:
                ...
                let instance = this.vm.instances()[data.Instance];
    ...
    public onError(exception: any) {
        console.log("Spatial2DTimeseriesView exception: " + JSON.stringify(exception));
        throw exception;
    }
