...
export class SimulationRunner implements
    ...
    public SimulateModel(ig: CRNInterfaces.IG, nodeId: string): Rx.Observable<SimViewer.IVisualizationUpdateMessage> {
        ...
        let space = { XAxis: axisDescriptionToValues(model.top.settings.spatial.nx, model.top.settings.spatial.xmax) };
        ...
        observables.simrun.subscribe(rundef => {
            ...
            rundef.values.subscribe(simdata => {
                let update: SimViewer.I1DSimStepData = {
                    Instance: simdata.instance,
                    Time: simdata.time,
                    Data: (<number[][]>simdata.values)
                }
                updates.onNext({ MessageType: SimViewer.VisualizationUpdateMessageType.SimulationStep1D, EncapsulatedUpdate: update });
            }, error => {
