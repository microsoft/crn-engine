...
class ContextGraph {
    ...
    customDrawLabel: ((context: CanvasRenderingContext2D, label: G.GLabel) => boolean) | null = null;
    ...
    private drawNode(context: CanvasRenderingContext2D, node: G.GNode): void {
        ...
        if (node.boundaryCurve != null)
            this.drawCurve(context, node.boundaryCurve, false);
        if (node.stroke != "")
    ...
    private drawEdge(context: CanvasRenderingContext2D, edge: G.GEdge): void {
        ...
        context.beginPath();
        if (edge.curve != null)
            this.drawCurve(context, edge.curve, false);
        if (edge.stroke != "")
