...
var WorkerConstructor: any;
export function SetWorkerConstructor(workerConstructor: any) { WorkerConstructor = workerConstructor; }
...
export abstract class GCurve implements ICurve {
    ...
    static ofCurve(curve: ICurve): GCurve {
        if (curve.curvetype == "Ellipse")
            return new GEllipse(<IEllipse><any>curve);
        else if (curve.curvetype == "Line")
            return new GLine(<ILine><any>curve);
        else if (curve.curvetype == "Bezier")
            return new GBezier(<IBezier><any>curve);
        else if (curve.curvetype == "Polyline")
            return new GPolyline(<IPolyline><any>curve);
        else if (curve.curvetype == "SegmentedCurve")
            return new GSegmentedCurve(<ISegmentedCurve><any>curve);
        else if (curve.curvetype == "RoundedRect")
            return new GRoundedRect(<IRoundedRect><any>curve);
        else
            throw "Invalid curve type " + curve.curvetype;
    }
}
...
export class GSegmentedCurve extends GCurve implements ICurve {
    ...
    constructor(segmentedCurve: ISegmentedCurve) {
        ...
        for (var i = 0; i < segmentedCurve.segments.length; i++) {
            var c = GCurve.ofCurve(segmentedCurve.segments[i]);
            if (c != null)
...
export class GLabel implements ILabel {
    ...
    constructor(label: ILabel) {
        if (typeof (label) == "string")
            ...
            this.bounds = label.bounds == null || label.bounds == GRect.zero ? GRect.zero : new GRect(label.bounds);
            this.tooltip = label.tooltip;
            this.content = label.content;
            this.fill = label.fill == null ? "Black" : label.fill;
        }
    }
}
...
export class GShape {
    ...
    static FromString(shape: string) {
        if (shape == "rect")
            ...
            throw "Undefined shape " + shape;
    }
    constructor(shape: IShape) {
        this.shape = shape == null || shape.shape == null ? "rect" : shape.shape;
        this.radiusX = shape == null || shape.radiusX == null ? 0 : shape.radiusX;
        this.radiusY = shape == null || shape.radiusY == null ? 0 : shape.radiusY;
        this.multi = shape == null || shape.multi == null ? 0 : shape.multi;
    }
    shape: string = "rect";
    radiusX?: number;
    radiusY?: number;
...
export class GNode implements INode {
    ...
    constructor(node: INode) {
        ...
        if (typeof (node.shape) == "string")
            this.shape = GShape.FromString(<string><any>node.shape);
        else if (node.shape != null)
            this.shape = new GShape(node.shape);
        var curve = node.boundaryCurve == null ? null : GCurve.ofCurve(node.boundaryCurve)
        if (curve != null)
            this.boundaryCurve = curve;
        if (typeof (node.label) == "string")
            this.label = new GLabel({ content: node.label });
        else if (node.label != null)
            this.label = new GLabel(node.label);
        this.labelMargin = node.labelMargin == null ? 5 : node.labelMargin;
        this.thickness = node.thickness == null ? 1 : node.thickness;
        this.fill = node.fill == null ? "White" : node.fill;
        this.stroke = node.stroke == null ? "Black" : node.stroke;
    }
...
export class GArrowHead implements IArrowHead {
    ...
    fill: boolean;
    dash?: string;
    style: string = "standard"; // standard|tee|diamond
    constructor(arrowHead: any)
    constructor(arrowHead: IArrowHead) {
        this.start = arrowHead.start == null ? GPoint.origin : arrowHead.start;
        this.end = arrowHead.end == null ? GPoint.origin : arrowHead.end;
        this.closed = arrowHead.closed == null ? false : arrowHead.closed;
        this.fill = arrowHead.fill == null ? false : arrowHead.fill;
        this.dash = arrowHead.dash;
        this.style = arrowHead.style == null ? "standard" : arrowHead.style;
    }
...
export interface IEdge extends IElement {
    ...
    target: string;
    label?: ILabel;
    arrowHeadAtTarget?: IArrowHead;
    arrowHeadAtSource?: IArrowHead;
    thickness: number;
    dash?: string;
    curve?: ICurve;
    stroke: string;
...
export class GEdge implements IEdge {
    ...
    constructor(edge: IEdge) {
        ...
        if (typeof (edge.label) == "string")
            this.label = new GLabel({ content: edge.label })
        else if (edge.label != null)
            this.label = new GLabel(edge.label);
        if (edge.arrowHeadAtTarget != null)
            this.arrowHeadAtTarget = new GArrowHead(edge.arrowHeadAtTarget);
        if (edge.arrowHeadAtSource != null)
            this.arrowHeadAtSource = new GArrowHead(edge.arrowHeadAtSource);
        this.thickness = edge.thickness == null ? 1 : edge.thickness;
        this.dash = edge.dash;
        if (edge.curve != null)
            this.curve = GCurve.ofCurve(edge.curve);
        this.stroke = edge.stroke == null ? "Black" : edge.stroke;
    }
}
...
class GNodeInternal {
    ...
    inEdges: string[] = [];
    selfEdges: string[] = [];
    constructor(n: GNode) { this.node = n; }
}
...
class GEdgeInternal {
    edge: GEdge;
    polyline: GPoint[];
    constructor(e: GEdge, p: GPoint[]) { this.edge = e; this.polyline = p; }
}
...
class MoveNodeToken extends MoveElementToken {
    ...
    public constructor(n: GNode, o: GPoint) { super(); this.node = n; this.originalBoundsCenter = o; }
}
...
class MoveEdgeLabelToken extends MoveElementToken {
    ...
    public constructor(l: GLabel, o: GPoint) { super(); this.label = l; this.originalLabelCenter = o; }
}
...
class MoveEdgeToken extends MoveElementToken {
    ...
    public constructor(e: GEdge, o: GPoint[], p: GPoint) { super(); this.edge = e; this.originalPolyline = o; this.originalPoint = p; }
}
...
export class GGraph implements IGraph {
    ...
    constructor(g?: IGraph) {
        if (GGraph.DebugLog)
            console.log("(debug) building a GGraph...");
        this.nodesMap = {};
        this.edgesMap = {};
        this.nodes = [];
        this.edges = [];
        this.boundingBox = (g == null || g.boundingBox == null) ? GRect.zero : new GRect(g.boundingBox);
        this.settings = new GSettings((g == null || g.settings == null) ? { transformation: { m00: -1, m01: 0, m02: 0, m10: 0, m11: -1, m12: 0 } } : g.settings);
        if (g != null && g.nodes != null)
            for (var i = 0; i < g.nodes.length; i++) {
                var inode: INode = g.nodes[i];
                if ((<ICluster>inode).children !== undefined) {
                    var gcluster = new GCluster(<ICluster>inode);
                    this.addNode(gcluster);
                }
                else {
    ...
    getInEdges(nodeId: string): string[] | null {
    ...
    addEdge(edge: GEdge): void {
        ...
        this.edgesMap[edge.id] = new GEdgeInternal(edge, []);
    ...
    createNodeBoundariesFromContext(context?: CanvasRenderingContext2D) {
        ...
        if (context == null) {
            ...
            if (ctx == null)
                throw "Unable to get a rendering context";
            context = ctx;
        }

        this.createNodeBoundaries(function (label) { return context == null ? GPoint.origin : GGraph.contextSizer(context, label); });
    ...
    createNodeBoundariesFromDiv(div?: HTMLDivElement) {
        var mydiv: HTMLDivElement | null = null;
        if (div == null) {
            div = mydiv = document.createElement('div');
            div.setAttribute('style', 'float:left');
            document.body.appendChild(div);
        }
        this.createNodeBoundaries(function (label) { return div == null ? GPoint.origin : GGraph.divSizer(div, label); });
        if (mydiv != null)
    ...
    public static defaultStyleString = "text { stroke: black; fill: black; stroke-width: 0; font-size: 15px; font-family: Verdana, Arial, sans-serif }";
    public static defaultStyle: any = { stroke: "black", fill: "black", strokeWidth: "0", fontSize: "15px", fontFamily: "Verdana, Arial, sans-serif" };
...
/** Creates node boundaries using the svgSizer on the given SVG element. You can also not give this function the SVG element, in which case it will use a temporary one. In this case, you can give it a style declaration that will be used for the temporary SVG element.*/
    createNodeBoundariesFromSVG(svg?: Element, style?: any) {
        var mysvg: Element | null = null;
        if (svg == null) {
            svg = mysvg = document.createElementNS('http://www.w3.org/2000/svg', 'svg');
            document.body.appendChild(svg);
        }
        if (style == null)
            style = GGraph.defaultStyle;
        if (style != null) {
            var fontProps = ["font", "fontFamily", "fontFeatureSettings", "fontSize", "fontSizeAdjust", "fontStretch", "fontStyle", "fontVariant", "fontWeight"];
            for (var p of fontProps)
                if (style[p] != null)
                    (<any>svg).style[p] = style[p];
        }
        this.createNodeBoundaries(function (label) { return GGraph.SVGSizer(<Element>svg, label); });
        if (mysvg != null)
    ...
    createNodeBoundariesForSVGInContainer(container: HTMLElement, style?: any) {
        var svg = document.createElementNS('http://www.w3.org/2000/svg', 'svg');
        container.appendChild(svg);
        this.createNodeBoundariesFromSVG(svg, style);
    ...
    createNodeBoundariesForSVGWithStyle(style: any) {
        this.createNodeBoundariesFromSVG(undefined, style);
    }
    ...
    private workerCallback(msg: MessageEvent) {
        ...
        if (GGraph.DebugLog)
            console.log("(debug) received worker response " + data.msgtype);
        if (data.msgtype == "RunLayout") {
            ...
            for (var nodeId in gs.nodesMap) {
                ...
                if (myNode == null) throw "Cannot find node " + nodeId;
                myNode.boundaryCurve = workerNode.node.boundaryCurve;
                if (myNode.label != null && workerNode.node.label != null)
            ...
            for (var edgeId in gs.edgesMap) {
                ...
                if (myEdge == null) throw "Cannot find edge " + edgeId;
                myEdge.curve = workerEdge.edge.curve;
                if (myEdge.label != null && workerEdge.edge.label != null)
            ...
            if (GGraph.DebugLog)
                console.log("(debug) invoking user callbacks");
            this.layoutCallbacks.fire();
            if (GGraph.DebugLog)
                console.log("(debug) user callbacks invoked");
        }
            ...
            for (var edgeId in gs.edgesMap) {
                ...
                if (routeEdgesMsg.edges == null || routeEdgesMsg.edges.length == 0 || routeEdgesMsg.edges.indexOf(workerEdge.edge.id) >= 0) {
                    ...
                    var myEdge: GEdge | null = edgeInternal.edge;
                    myEdge.curve = workerEdge.edge.curve;
                    edgeInternal.polyline = [];
                    if (myEdge.label != null && workerEdge.edge.label != null)
                        myEdge.label.bounds = workerEdge.edge.label.bounds;
                    if (myEdge.arrowHeadAtSource != null)
                        myEdge.arrowHeadAtSource = workerEdge.edge.arrowHeadAtSource;
            ...
            edge.curve = curve;
            if (edge.arrowHeadAtSource != null && setPolylineMsg.sourceArrowHeadStart != null)
                edge.arrowHeadAtSource.start = JSON.parse(setPolylineMsg.sourceArrowHeadStart);
            if (edge.arrowHeadAtSource != null && setPolylineMsg.sourceArrowHeadEnd != null)
                edge.arrowHeadAtSource.end = JSON.parse(setPolylineMsg.sourceArrowHeadEnd);
            if (edge.arrowHeadAtTarget != null && setPolylineMsg.targetArrowHeadStart != null)
                edge.arrowHeadAtTarget.start = JSON.parse(setPolylineMsg.targetArrowHeadStart);
            if (edge.arrowHeadAtTarget != null && setPolylineMsg.targetArrowHeadEnd != null)
    ...
    private ensureWorkerReady(): void {
        ...
        if (this.worker == null) {
            ...
            if (this.worker == null) throw "Invalide WorkerConstructor";
    ...
    public beginLayoutGraph(): void {
        ...
        if (this.worker == null) throw "Worker is null";
        if (GGraph.DebugLog)
            console.log("(debug) sending layout request");
    ...
    public getControlPointAt(edge: GEdge, point: GPoint): GPoint | null {
        var points = this.getPolyline(edge.id);
        // Iterate over points, comparing the squared distance.
        var ret: GPoint | null = points[0];
    ...
    public startMoveElement(el: IElement, mousePoint: GPoint) {
        if (el instanceof GNode) {
            ...
            var node = <GNode>el;
            if (node.boundaryCurve == null) throw "Invalid node boundary curve";
            var mnt = new MoveNodeToken(node, node.boundaryCurve.getCenter());
            ...
            var label = <GLabel>el;
            var melt = new MoveEdgeLabelToken(label, label.bounds.getCenter());
            ...
            if (point != null) {
                var met = new MoveEdgeToken(edge, this.getPolyline(edge.id), point);
    ...
    public moveElements(delta: GPoint) {
        for (var i in this.moveTokens) {
            ...
            if (token instanceof MoveNodeToken) {
                ...
                if (ntoken.node.boundaryCurve == null) throw "Invalide node boundary curve";
    ...
    private delayCheckRouteEdges: (() => void) | null = null;
    ...
    private delayCheckRebuildEdge: (() => void) | null = null;
    ...
    private getOutdatedEdges(): string[] {
        ...
        for (var t in this.moveTokens) {
            ...
            if (token instanceof MoveNodeToken) {
                ...
                var ntoken = <MoveNodeToken>token;
                var inEdges = this.getInEdges(ntoken.node.id);
                var outEdges = this.getOutEdges(ntoken.node.id);
                var selfEdges = this.getSelfEdges(ntoken.node.id);
                if (inEdges == null || outEdges == null || selfEdges == null) throw "Cannot retrieve edges for node " + ntoken.node.id;
                var nEdges = inEdges.concat(outEdges).concat(selfEdges);
    ...
    private makePolyline(edge: GEdge): GPoint[] {
        ...
        if (source.node.boundaryCurve == null) throw "Invalide node boundary curve";
        ...
        if (target.node.boundaryCurve == null) throw "Invalide node boundary curve";
    ...
    public getPolyline(edgeID: string): GPoint[] {
        ...
        if (edgeInternal.polyline.length == 0)
