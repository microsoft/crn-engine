...
class IDDSVGGraph extends SVGGraph {
    ...
    private static msaglPlot: any = function (this: any, graph: IDDSVGGraph, jqDiv: any, master: any) {
        ...
        var _svgCnt: HTMLElement | null = null;
    ...
    constructor(container: HTMLElement, graph?: G.GGraph) {
        ...
        IDDSVGGraph.msaglPlot.prototype = new (<any>InteractiveDataDisplay.Plot);
        ...
        // Set up container size monitoring. I need to perform some layout operations any time the container is resized.
    ...
    /** If the container size has changed, I invoked updateLayouts. */
    ...
    private redrawGraph(): boolean {
        if (G.GGraph.DebugLog)
            console.log("(debug) redrawGraph invoked");
        this.svg = this.gplot.svg;
        if (this.svg == null) {
            if (G.GGraph.DebugLog)
                console.log("(debug) redrawGraph refused, no svg");
            return false;
        }
        ...
        if (this.graph == null) {
            if (G.GGraph.DebugLog)
                console.log("(debug) redrawGraph refused, graph is null");
            return false;
        }
        return true;
    }
    ...
    drawGraph(): void {
        if (G.GGraph.DebugLog)
        console.log("(debug) drawGraph invoked");
        // Populate the graph.
        if (this.graph == null || !this.redrawGraph() || this.svg == null) {
            if (G.GGraph.DebugLog) {
                console.log("(debug) drawGraph refused");
                if (this.graph == null)
                    console.log("(debug) this.graph is null");
                if (this.svg == null)
                    console.log("(debug) this.svg is null");
            }
            return;
        }
        ...
        var offsetX = bbox.x;
        var offsetY = bbox.y;
        var swidth = this.svg.getAttribute('width');
        var sheight = this.svg.getAttribute('height');
        var cwidth = swidth == null ? 0 : parseFloat(swidth);
        var cheight = sheight == null ? 0 : parseFloat(sheight);
    ...
    protected onMouseDblClick(e: MouseEvent) {
        if (this.svg != null && this.graph != null && !this.isEditingEdge())
