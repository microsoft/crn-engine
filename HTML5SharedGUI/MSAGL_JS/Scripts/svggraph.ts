...
class RenderEdgeLabel extends RenderElement {
    ...
    public edge: G.GEdge;
    public getGeometryElement() {
        if (this.edge.label == null) throw "Invalid edge label";
...
class SVGGraph {
    ...
    private edgeRoutingCallback: ((edges?: string[]) => void) | null = null;
    private layoutStartedCallback: (() => void) | null = null;
    private workStoppedCallback: (() => void) | null = null;

    private workingSpan: HTMLSpanElement;

    public getGraph(): G.GGraph | undefined { return this.graph; }
    public setGraph(graph: G.GGraph) {
        if (this.graph != null) {
            if (this.edgeRoutingCallback != null)
                this.graph.edgeRoutingCallbacks.remove(this.edgeRoutingCallback);
            if (this.layoutStartedCallback != null)
                this.graph.layoutStartedCallbacks.remove(this.layoutStartedCallback);
            if (this.workStoppedCallback != null)
    ...
    public getSVGString(): string {
        if (this.svg == null || this.graph == null)
            return "";
    ...
    customDrawLabel: ((svg: Element, parent: Element, label: G.GLabel, owner: G.IElement) => boolean) | null = null;
    ...
    private drawNode(parent: Element, node: G.GNode): void {
        if (this.svg == null || node.boundaryCurve == null)
            return;
        var g = <SVGGElement>document.createElementNS(SVGGraph.SVGNS, "g");
        var nodeCopy = node;
        var that = this;
        g.onclick = function () { that.onNodeClick(nodeCopy); };
        var pathString = this.pathCurve(node.boundaryCurve, false) + "Z";
    ...
    private drawEdge(parent: Element, edge: G.GEdge): void {
        if (this.svg == null || edge.curve == null)
            return;
        var g = <SVGGElement>document.createElementNS(SVGGraph.SVGNS, "g");
        var edgeCopy = edge;
        var that = this;
        g.onclick = function () { that.onEdgeClick(edgeCopy); };
        var pathString = this.pathCurve(edge.curve, false);
        var path = document.createElementNS(SVGGraph.SVGNS, "path");
        path.setAttribute("d", pathString);
    ...
    public styleString: string = G.GGraph.defaultStyleString;
    public style: any = G.GGraph.defaultStyle;
    ...
    populateGraph(): void {
        if (G.GGraph.DebugLog)
            console.log("(debug) populateGraph invoked");
        if (this.svg == null || this.graph == null)
            return;
        if (this.styleString != null) {
            var style = document.createElementNS(SVGGraph.SVGNS, "style");
            var styleText = document.createTextNode(this.styleString);
        ...
        for (var i = 0; i < this.graph.nodes.length; i++) {
            if (G.GGraph.DebugLog)
                console.log("(debug) drawing node " + this.graph.nodes[i].id);
            this.drawNode(this.svg, this.graph.nodes[i]);
        }
        for (var i = 0; i < this.graph.edges.length; i++) {
            if (G.GGraph.DebugLog)
                console.log("(debug) drawing edge " + this.graph.edges[i].id);
            this.drawEdge(this.svg, this.graph.edges[i]);
        }
    }

    drawGraph(): void {
        while (this.svg != null && this.svg.firstChild != null)
            this.svg.removeChild(this.svg.firstChild);
        if (this.grid != null && this.svg != null)
    ...
    private redrawElement(el: RenderElement) {
        ...
        if (el instanceof RenderNode) {
            ...
            if (renderEdgeLabel.edge.label != null)
    ...
    private mousePoint: G.GPoint | null = null;
    /** The graph element that's currently under the mouse cursor, if any. */
    private elementUnderMouseCursor: RenderElement | null = null;
    ...
    public getGraphPoint(e: MouseEvent) {
        ...
        if (matrix == null) throw "Cannot get a transformation matrix";
    ...
    private dragElement: RenderElement | null = null;
    /** The point where the mouse button went down. Used to establish a delta while dragging. */
    private mouseDownPoint: G.GPoint | null = null;
    ...
    private beginDrag() {
        if (this.elementUnderMouseCursor == null || this.graph == null || this.mouseDownPoint == null)
            return;
    ...
    private doDrag() {
        if (this.dragElement == null || this.mousePoint == null || this.graph == null || this.mouseDownPoint == null)
            return;
        // Compute the delta.
    ...
    private edgeEditModeTimeout: number | null = null;
    /** The edge that's currently being edited. */
    private edgeEditEdge: RenderEdge | null = null;
    ...
    private drawPolylineCircles() {
        if (this.edgeEditEdge == null || this.graph == null)
            return;
    ...
    private exitEdgeEditMode() {
        ...
        if (this.edgeEditModeTimeout != null)
            clearTimeout(this.edgeEditModeTimeout);
    ...
    private beginExitEdgeEditMode() {
        ...
        var t = window.setTimeout(() => that.exitEdgeEditMode(), SVGGraph.ExitEdgeModeTimeout);
        this.edgeEditModeTimeout = t;
    }

    /** Handles an attempt to insert/remove an edge control point. */
    private edgeControlPointEvent(point: G.GPoint) {
        if (this.graph == null || this.edgeEditEdge == null)
            return;
