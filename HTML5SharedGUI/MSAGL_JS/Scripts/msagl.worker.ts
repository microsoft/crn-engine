...
if (G.GGraph.DebugLog)
    console.log("(debug worker) loading...");

// Load the SharpKit and transpiled Msagl scripts.
var script_sharpkitPre = require("raw-loader!./sharpkit_pre.js");
var script_jsclr = require("raw-loader!./jsclr.min.js");
var script_Msagl = require("raw-loader!./Microsoft.Msagl.min.js");
var script_sharpkitPost = require("raw-loader!./sharpkit_post.js");
// Get rid of "use strict", or the eval call won't work due to scoping rules.
script_jsclr = script_jsclr.replace(/('|")use strict('|");?/, '');
script_Msagl = script_Msagl.replace(/('|")use strict('|");?/, '');
// Run all of them.
eval.call(null, script_sharpkitPre);
eval.call(null, script_jsclr);
eval.call(null, script_Msagl);
eval.call(null, script_sharpkitPost);
...
class LayoutWorker {
    ...
    private getGCurve(curve: any): G.GCurve {
        ...
        throw "Cannot generate a MSAGL curve from " + JSON.stringify(curve);
    }
    ...
    setPolyline(edge: string, points: G.GPoint[]): SetPolylineResult {
        ...
        var sourceArrowHeadStart = hasSourceArrowhead ? this.getGPoint(mcurve.get_Start()) : undefined;
        var sourceArrowHeadEnd = hasSourceArrowhead ? this.getGPoint(medge.get_EdgeGeometry().get_SourceArrowhead().get_TipPosition()) : undefined;
        var targetArrowHeadStart = hasTargetArrowhead ? this.getGPoint(mcurve.get_End()) : undefined;
        var targetArrowHeadEnd = hasTargetArrowhead ? this.getGPoint(medge.get_EdgeGeometry().get_TargetArrowhead().get_TipPosition()) : undefined;
...
export function handleMessage(e: any): void {
    ...
    var answer: M.Response | undefined = undefined;
    switch (message.msgtype) {
        ...
        case "RouteEdges":
            {
                try {
                    var edges = (<M.Req_RouteEdges>message).edges;
        ...
        case "SetPolyline":
            {
                ...
                try {
                    ...
                    answer = {
                        ...
                        sourceArrowHeadStart: result.sourceArrowHeadStart == null ? undefined : JSON.stringify(result.sourceArrowHeadStart),
                        sourceArrowHeadEnd: result.sourceArrowHeadEnd == null ? undefined : JSON.stringify(result.sourceArrowHeadEnd),
                        targetArrowHeadStart: result.targetArrowHeadStart == null ? undefined : JSON.stringify(result.targetArrowHeadStart),
                        targetArrowHeadEnd: result.targetArrowHeadEnd == null ? undefined : JSON.stringify(result.targetArrowHeadEnd)
                    };
                }
                catch (e) {
                    if (e.message != null)
...
if (G.GGraph.DebugLog)
    console.log("(debug worker) loaded");