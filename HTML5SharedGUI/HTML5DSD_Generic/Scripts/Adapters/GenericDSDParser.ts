...
export interface IResultStreams<TCalcOptions> {
    result: Rx.Observable<IResult<TCalcOptions>>,
}

//Uses ModellingEngine.ts layer to aquire parsed CRN code
export abstract class Parser<TCalcSettings, TOptions> implements ParseOperation.IParser<DSDParseOperation.IParsed<TCalcSettings>, TOptions> {
    abstract getCRNStreams(code: string, options: TOptions, server?: boolean): IResultStreams<TCalcSettings>;
    ...
    public TryParse(code: string, options: TOptions, server?: boolean): JQueryDeferred<ParseOperation.IParsingResults<DSDParseOperation.IParsed<TCalcSettings>>> {
        ...
        var crnStreams = this.getCRNStreams(code, options, server);
